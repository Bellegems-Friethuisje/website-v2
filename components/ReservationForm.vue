<template>
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
    ...
    <form v-else @submit.prevent="submit" novalidate class="space-y-5">
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            type="text"
            autocomplete="name"
            :placeholder="t({ nl: 'Uw naam', fr: 'Votre nom' })"
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{
            t({ nl: "E-mailadres", fr: "Adresse e-mail" })
          }}</label>
          <input
            v-model="form.email"
            type="email"
            autocomplete="email"
            :placeholder="t({ nl: 'uw@email.be', fr: 'votre@email.be' })"
      ...
      <div class="relative" ref="pickerAnchorEl">
        ...
        >
          <span :class="pickerLabel ? 'text-gray-900' : 'text-gray-400'">
            {{
              pickerLabel ||
              t({
                nl: "Kies datum en tijdstip",
                fr: "Choisissez date et heure",
              })
            }}
          </span>
          <svg
        ...
        <p v-if="errors.date || errors.time" class="text-red-400 text-xs mt-1">
          {{ errors.date || errors.time }}
        </p>
        ...
        <Transition name="picker">
          ...
          >
            <!-- Mobile header -->
            <div
              class="flex items-center justify-between px-4 py-4 border-b border-gray-100 sm:hidden"
            >
              <button
                type="button"
                @click="
                  pickerStep === 'time' ? (pickerStep = 'date') : closePicker()
                "
            ...
            <div v-else class="flex-1 overflow-auto p-4 flex flex-col gap-4">
              ...
              <button
                ...
                class="flex items-center gap-2 text-sm text-orange-500 font-semibold hover:text-orange-600 transition-colors self-start"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-3.5 w-3.5"
              ...
              <!-- No services warning -->
              <div
                v-if="isSelectedToday && !hasUpcomingService"
      ...
      <div>
        ...
        <input
          ...
          type="number"
          min="10"
          :placeholder="t({ nl: 'Bijv. 15', fr: 'Ex. 15' })"
          :class="inputClass('quantity')"
        />
        <p v-if="errors.quantity" class="text-red-400 text-xs mt-1">
          {{ errors.quantity }}
        </p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          :placeholder="
            t({
              nl: 'Allergieën, speciale wensen...',
              fr: 'Allergies, souhaits particuliers...',
            })
          "
      ...
      >
        ...
        {{
          loading
            ? t({ nl: "Verzenden...", fr: "Envoi..." })
            : t({ nl: "Reservatie aanvragen", fr: "Demander une réservation" })
        }}
      </button>
    </form>
  </div>
  ...
  <div
    v-if="showPicker"
    class="hidden sm:block fixed inset-0 z-20"
    @click="closePicker"
  />
</template>
...
const hasUpcomingService = computed(() => {
  ...
  return selectedDayServices.value.some(
    (s) => getServiceStatus(s) === "upcoming",
  );
});
    ...
    for (let mins = service.start; mins < service.end; mins += 30) {
      slots.push({
        ...
        disabled:
          status === "passed" ||
          status === "ongoing" ||
          (isSelectedToday.value && mins <= currentMinutes.value),
      });
    }
...
function inputClass(field: string) {
  const base =
    "w-full px-4 py-3 rounded-xl border text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400";
...
function validate(): boolean {
  ...
  if (!form.value.date)
    e.date = t({
      nl: "Kies een datum en tijdstip",
      fr: "Choisissez une date et une heure",
    });
  else if (!form.value.time)
    e.time = t({
      nl: "Kies nog een tijdstip",
      fr: "Choisissez encore une heure",
    });
